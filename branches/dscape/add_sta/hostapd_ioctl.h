...
struct prism2_hostapd_param {
	...
	union {
		struct {
			 ...
			 */
			__u8 atheros_super_ag;
			__u8 atheros_xr_mode;
			__u8 wds_flags;
#define IEEE80211_STA_DYNAMIC_ENC BIT(0)
		...
		struct {
			...
			__u32 rx_bytes;
			__u32 tx_bytes;
			__u32 current_tx_rate; /* in 100 kbps */
                        __u32 channel_use;
                        __u32 flags;
			__u32 num_ps_buf_frames;
			__u32 tx_retry_failed;
			__u32 tx_retry_count;
			__u32 last_rssi;
			__u32 last_ack_rssi;
		} get_info_sta;
		struct {
			__u8 alg[HOSTAP_CRYPT_ALG_NAME_LEN];
			__u32 flags;
			__u32 err;
			__u8 idx;
#define HOSTAP_SEQ_COUNTER_SIZE 8
			__u8 seq_counter[HOSTAP_SEQ_COUNTER_SIZE];
			__u16 key_len;
			__u8 key[0] ALIGNED;
		} crypt;
		struct {
		...
		struct {
			__u16 head_len;
			__u16 tail_len;
			__u8 data[0] ALIGNED; /* head_len + tail_len bytes */
		} beacon;
		struct {
			__u16 num_modes;
			__u16 flags;
			__u8 data[0] ALIGNED; /* num_modes * feature data */
                } hw_features;
                struct {
                        __u8  now;
                        __s8  our_mode_only;
                        __s16 last_rx;
                        __u16 channel;
                        __s16 interval; /* seconds */
                        __s32 listen;   /* microseconds */
                } scan;
		struct {
...
#define WPA_TRIGGER_TX_SKIP_SEQ BIT(6)
		...
		struct {
			__u16 mode; /* MODE_* */
			__u16 num_supported_rates;
			__u16 num_basic_rates;
			__u8 data[0] ALIGNED; /* num_supported_rates * __u16 +
				             * num_basic_rates * __u16 */
                } set_rate_sets;
                struct {
			__u8 type; /* WDS, VLAN, etc */
			__u8 name[IFNAMSIZ];
                        __u8 data[0] ALIGNED;
                } if_info;
                struct dot11_counters {
                        __u32 dot11TransmittedFragmentCount;
                        __u32 dot11MulticastTransmittedFrameCount;
                        __u32 dot11FailedCount;
                        __u32 dot11ReceivedFragmentCount;
                        __u32 dot11MulticastReceivedFrameCount;
                        __u32 dot11FCSErrorCount;
                        __u32 dot11TransmittedFrameCount;
                        __u32 dot11WEPUndecryptableCount;
			__u32 dot11ACKFailureCount;
			__u32 dot11RTSFailureCount;
			__u32 dot11RTSSuccessCount;
                } dot11CountersTable;
		struct {
#define LOAD_STATS_CLEAR BIT(1)
		...
		struct {
			...
			__u16 chan;
			__u32 flag;
                        __u8  power_level; /* regulatory limit in dBm */
                        __u8  antenna_max;
		} set_channel_flag;
                struct {
                        __u32 rd;
		...
		struct {
			...
			__u32 cw_min;
			__u32 cw_max;
			__u32 burst_time; /* maximum burst time in 0.1 ms, i.e.,
					 * 10 = 1 ms */
		} tx_queue_params;
		struct {
			__u32 bss_count;
			__u8 bssid_mask[ETH_ALEN];
		} set_bss;
...
#define MLME_STA_DISASSOC 1
		...
		*/
		...
		struct {
			__u8 dummy[80]; /* Make sizeof() this struct large enough

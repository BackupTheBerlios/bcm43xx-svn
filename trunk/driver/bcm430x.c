...
static void bcm430x_shm_write32(struct bcm430x_private *bcm, u32 control, u32 val)
{
	bcm430x_write32(bcm, control, BCM430x_SHM_CONTROL);
...
static void bcm430x_switch_core(struct bcm430x_private *bcm, u32 core)
{
	...
	bcm430x_pci_read_config_32(bcm->pci_dev, BCM430x_REG_ACTIVE_CORE, &real);
	...
	while (real != wanted && i < BCM430x_SWITCH_CORE_MAX_RETRIES) {
		bcm430x_pci_write_config_32(bcm->pci_dev, BCM430x_REG_ACTIVE_CORE, wanted);
		bcm430x_pci_read_config_32(bcm->pci_dev, BCM430x_REG_ACTIVE_CORE, &real);
		i++;
	}
...
static int bcm430x_init_board	(struct pci_dev *pdev,
